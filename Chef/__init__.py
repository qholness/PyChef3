"""Python 3 revision of Chef (Esoteric Language) Python Interpreter
"""
...
import copy
import datetime
from Chef.dishes import dishes, serving_dishes
start = datetime.datetime.now()
class Chef:
	def __init__(self, script, mixingbowls = {0: []}, **kwargs):		
		self.script = script
		self.origscript = script
		self.mixingbowls = copy.deepcopy(mixingbowls)
		self.bakingdishes = {}
		self.dishes = set(kwargs.get("dishes", set())) | dishes
		self.serving_dishes = set(kwargs.get("serving_dishes", set())) | serving_dishes
		self.recipename = kwargs.get("recipename")
	def parse(self):
		for d in self.dishes:
			self.script = self.script.replace(d, "mixing bowl")
		for s in self.serving_dishes:
			self.script = self.script.replace(s, "baking dish")
		recipename = re.match("(.*?)\.\n\n", self.script) if not self.recipename else self.recipename
		if(recipename == None):
			print("Invalid recipe name")
			sys.exit(-1)
		try:
			self.script = re.sub(recipename.group(), "", self.script)
		except AttributeError:
			self.script = re.sub(recipename, "", self.script)
		...
		self.ingredients = re.findall("(([0-9]*) ?(k?g|pinch(?:es)?|m?l|dash(?:es)?|cups?|teaspoons?|tablespoons?)? ?([a-zA-Z ]+)\n)", self.script)
		#Match ingredients.
		self.ingredientlist = {}
	...
	def put(self, mixingbowl, value):
		...
		if mixingbowl == None:
			...
			if not self.mixingbowls.get(key):
	...
	def execute(self, text, loop=False):
		...
		for ex in excode:
			...
			fold = re.search("Fold ([a-zA-Z ]+) into (1st|2nd|3rd|[0-9]+th)? ?mixing bowl", ex)
			if fold != None:				
			...
			if clean != None:
				if clean.group(1) == None:
					...
					if self.mixingbowls.get(clean.group(1)[:-2]):
			...
			if mix != None:
				if mix.group(1) == None:
					...
					if self.mixingbowls.get(mix.group(1)[:-2]):
			...
			if(fridge != None):
				if self.ingredientlist.get(fridge.group(1)):
			...
			if pour != None:				
				...
				if not self.bakingdishes.get(key2):					
			...
			if verb != None:				
				if "until" in verb.group():
					continue
				if not self.ingredientlist.get(verb.group(2)):
					continue
				...
				for j in self.bakingdishes[i]:
					...
					if j[1] == "liquid":
						try:
							...
							raise OverflowError("Unicode conversion error.")
						except ValueError as v:
							raise ValueError("Unicode value error.")
					output += str(value)
		return output
